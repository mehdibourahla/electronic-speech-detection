...
def yamnet(features, params):
    ...
    embeddings_layer10 = None
    for i, (layer_fun, kernel, stride, filters) in enumerate(_YAMNET_LAYER_DEFS):
        net = layer_fun("layer{}".format(i + 1), kernel, stride, filters, params)(net)
        if i == 8:
            embeddings_layer10 = net
    embeddings_final = layers.GlobalAveragePooling2D()(net)
    logits = layers.Dense(units=params.num_classes, use_bias=True)(embeddings_final)
    predictions = layers.Activation(activation=params.classifier_activation)(logits)
    return predictions, embeddings_final, embeddings_layer10
...
def yamnet_frames_model(feature_params):
    ...
    spectrogram, patches = features_lib.waveform_to_log_mel_spectrogram_patches(
        tf.squeeze(waveform, axis=0), feature_params
    )
    predictions, embeddings_final, embeddings_layer10 = yamnet(patches, feature_params)
    frames_model = Model(
        name="yamnet_frames",
        inputs=waveform,
        outputs=[predictions, embeddings_final, embeddings_layer10, spectrogram],
    )
    return frames_model
